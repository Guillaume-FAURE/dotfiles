...
graph
    subgraph 1[" "]
    direction LR
    A[Sequence of character]-->B[Sequence of tokens]
    B-->C[Token's tree]
    end
    subgraph 2[" "]
    direction RL
    D[Datatype Conversion]-->E[Intermediate code]
    E-->F["Code optimized (independent from machine)"]
    end
    subgraph 3 [" "]
    direction LR
    F-->G[Code in target language]
    G-->H[Code optimized in machine code]
    end
    C-->D
```
