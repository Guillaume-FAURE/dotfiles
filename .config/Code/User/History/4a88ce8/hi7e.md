...
It don't works because of the call of the FACTORIAL by the macro FACTORIAL, the pre-processor don't pass multiple times by the macros so a recursive function as a macro can't work.

### 10. If it doesn't work: Explain how the C preprocessor would have to be modified for the macro to work!

For the macro to work, we have to abandonned the idea of a recursive function as a macro or to make the preprocessor go multiple time through macro as a normal processor but the principle of the preprocessor don't have an interest anymore.
